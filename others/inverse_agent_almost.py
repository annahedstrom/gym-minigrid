...
class InverseAgentClass():
    ...
    def set_risk_tolerance(self):
        ...
        risky_nei = np.unique([s for (s, risk) in self.risk_types if risk == "?"])
        print(risky_nei)
        # Identify concerned neighbours.
        for i in range(len(risky_nei)):
            self.y = []
            self.x = []
            self.neighbours2d = 0
            self.neighbours = []
            idy, idx = (int(risky_nei [i] % self.gridSize), int(risky_nei [i] / self.gridSize))
            neigh_idx = np.vstack((idx - 1, idx + 1, idx, idx))
            neigh_idy = np.vstack((idy, idy, idy - 1, idy + 1))
            for j in range(len(neigh_idx)):
                self.x.append(neigh_idx[j])
                self.y.append(neigh_idy[j])
            self.x = np.ndarray.flatten(np.array(self.x)); self.y = np.ndarray.flatten(np.array(self.y))
            self.neighbours = tuple(zip(self.x, self.y,))
            ...
            for k in range(len(self.neighbours)):
                for a in range(self.num_actions):
                    ...
                    if turn == 0: # and self.x[a] >= 0 and self.y[a] >= 0:
                        self.pi[k, 3] = 0  # Down action of up-side state is forbidden.
                    if turn == 1: # and self.x[a] >= 0 and self.y[a] >= 0:
                        self.pi[k, 2] = 0  # Up action of down-side state is forbidden.
                    if turn == 2: # and self.x[a] >= 0 and self.y[a] >= 0:
                        self.pi[k, 1] = 0  # Right action of left-side state is forbidden.
                    if turn == 3: # and self.x[a] >= 0 and self.y[a] >= 0:
                        self.pi[k, 0] = 0  # Left action of right-side state is forbidden.
                turn += 1
